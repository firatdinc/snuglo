...
- **LeaderboardViewModel.swift** (yeni — `Features/Leaderboard/`): `enum LeaderboardLoadState: Equatable` (6 case). `load()` async: idle ise authenticate, authState'e göre GC veya not-signed-in/error. `fallbackEntries`: 4 simüle oyuncu + yerel oyuncu, board sıralamasına göre sıralı, 1-bazlı yeniden sıralama.
